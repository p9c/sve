...
import (
	...
	// "github.com/p9c/sve/pkg/internal/htmlx"
	// "github.com/p9c/sve/pkg/internal/htmlx/atom"
	// "golang.org/x/net/html"
	// "golang.org/x/net/html/atom"
	"github.com/p9c/sve/pkg/html"
	"github.com/p9c/sve/pkg/html/atom"
)
...
func (p *ParserGo) visitOverall(state *parseGoState) error {
	...
	fmt.Fprintf(&state.goBuf, "// DO NOT EDIT: This file was generated by engine. Please regenerate instead of editing or add additional code in a separate file.\n\n")
	...
	fmt.Fprintf(&state.buildBuf, "func (c *%s) Build(vgin *sve.BuildIn) (vgout *sve.BuildOut) {\n", p.StructType)
	fmt.Fprintf(&state.buildBuf, "    \n")
	fmt.Fprintf(&state.buildBuf, "    vgout = &sve.BuildOut{}\n")
	fmt.Fprintf(&state.buildBuf, "    \n")
	fmt.Fprintf(&state.buildBuf, "    var vgiterkey interface{}\n")
	fmt.Fprintf(&state.buildBuf, "    _ = vgiterkey\n")
	fmt.Fprintf(&state.buildBuf, "    var vgn *sve.VGNode\n")
	// fmt.Fprintf(&buildBuf, "    var vgparent *sve.VGNode\n")
...
func (p *ParserGo) visitHTML(state *parseGoState, n *html.Node) error {
	...
	pOutputTag(state, n)
	// fmt.Fprintf(&state.buildBuf, "vgn = &sve.VGNode{Type:sve.VGNodeType(%d),Data:%q,Attr:%#v}\n", n.Type, n.Data, staticVGAttr(n.Attr))
	...
	for _, k := range dynExprMapKeys {
		...
		fmt.Fprintf(&state.buildBuf, "vgn.Attr = append(vgn.Attr, engine.VGAttribute{Key:%q,Val:fmt.Sprint(%s)})\n", k, valExpr)
	}
...
func (p *ParserGo) visitJS(state *parseGoState, n *html.Node) error {
	...
	if n.FirstChild != nil {
		...
		for childN := n.FirstChild; childN != nil; childN = childN.NextSibling {
			...
			fmt.Fprintf(&state.buildBuf, "vgn.AppendChild(&sve.VGNode{Type:sve.VGNodeType(%d),Data:%q,Attr:%#v})\n", childN.Type, childN.Data, staticVGAttr(childN.Attr))
		}
		fmt.Fprintf(&state.buildBuf, "}\n")
	}
...
func (p *ParserGo) visitNodeJustElement(state *parseGoState, n *html.Node) error {
	...
	for _, k := range propExprMapKeys {
		...
		fmt.Fprintf(&state.buildBuf, "{b, err := json.Marshal(%s); if err != nil { return nil, err }; vgn.Prop = append(vgn.Prop, engine.VGProperty{Key:%q,Val:json.RawMessage(b)})}\n", valExpr, k)
	}
	...
	for _, k := range eventKeys {
		expr := eventMap[k]
		fmt.Fprintf(&state.buildBuf, "vgn.DOMEventHandlerSpecList = append(vgn.DOMEventHandlerSpecList, engine.DOMEventHandlerSpec{\n")
		fmt.Fprintf(&state.buildBuf, "EventType: %q,\n", k)
		fmt.Fprintf(&state.buildBuf, "Func: func(event *sve.DOMEvent) { %s },\n", expr)
...
func (p *ParserGo) visitNodeText(state *parseGoState, n *html.Node) error {
	...
	fmt.Fprintf(&state.buildBuf, "vgn = &sve.VGNode{Type:sve.VGNodeType(%d),Data:%q}\n", n.Type, n.Data)
	fmt.Fprintf(&state.buildBuf, "vgparent.AppendChild(vgn)\n")
...
func (p *ParserGo) visitNodeComponentElement(state *parseGoState, n *html.Node) error {
	...
	// find a way to re-use the earlier value - but it will get real annoying real fast when .sve fiiles that didn't
	// change have .go files marked as changed just because this ID was generated differently.
	compKeyID := component.MakeCompKeyIDNowRand()
	...
	keyExpr := vgIfExpr(n)
	if keyExpr != "" {
		fmt.Fprintf(&state.buildBuf, "vgcompKey := engine.CompKey{ID:0x%X, IterKey: %s}\n", compKeyID, keyExpr)
	} else {
		fmt.Fprintf(&state.buildBuf, "vgcompKey := engine.CompKey{ID:0x%X, IterKey: vgiterkey}\n", compKeyID)
	}
	...
	fmt.Fprintf(&state.buildBuf, "vgn = &sve.VGNode{Component:vgcomp}\n")
	fmt.Fprintf(&state.buildBuf, "vgparent.AppendChild(vgn)\n")
