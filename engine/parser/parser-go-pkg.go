...
type ParserGoPkgOpts struct {
	SkipRegisterComponentTypes bool // indicates func init() { engine.RegisterComponentType(...) } code should not be emitted in each file
...
`), 0644)
			...
			// 	rootInst, err := engine.New(&Root{}, nil)
			// 	if err != nil {
			// 		log.Fatal(err)
			// 	}

			// 	env := engine.NewJSEnv("#root_mount_parent", rootInst, engine.RegisteredComponentTypes())
	...
	for _, fn := range sveFileNames {
		...
		err := func() error {
			...
			// 	fmt.Fprintf(f, "\nfunc (ct *%s) NewData(props engine.Props) (interface{}, error) { return &%s{}, nil }\n",
