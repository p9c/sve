...
func (b *BuildOut) AppendCSS(nlist ...*vnode.VGNode) {
...
func (b *BuildOut) AppendJS(nlist ...*vnode.VGNode) {
nlistloop:
	for _, n := range nlist {
		for _, js := range b.JS {
			if !ssNodeEq(n, js) {
...
func ssText(n *vnode.VGNode) string {
...
func ssNodeEq(n1, n2 *vnode.VGNode) bool {
	if n1.Type != n2.Type {
		return false
	}
	if n1.Data != n2.Data {
...
type ComponentType interface {
	BuildVDOM(data interface{}) (vdom *vnode.VGNode, css *vnode.VGNode, reterr error) // based on the given data, build the VGNode tree
