...
func (p *ParserGo) visitOverall(state *parseGoState) error {
	...
	fmt.Fprintf(&state.goBuf, "// DO NOT EDIT: This file was generated by sve. Please regenerate instead of editing or add additional code in a separate file.\n\n")
...
func (p *ParserGo) visitHTML(state *parseGoState, n *html.Node) error {
	...
	for _, k := range dynExprMapKeys {
		...
		fmt.Fprintf(&state.buildBuf, "vgn.Attr = append(vgn.Attr, sve.VGAttribute{Key:%q,Val:fmt.Sprint(%s)})\n", k, valExpr)
	}
...
func (p *ParserGo) visitNodeJustElement(state *parseGoState, n *html.Node) error {
	...
	for _, k := range propExprMapKeys {
		...
		fmt.Fprintf(&state.buildBuf, "{b, err := json.Marshal(%s); if err != nil { return nil, err }; vgn.Prop = append(vgn.Prop, sve.VGProperty{Key:%q,Val:json.RawMessage(b)})}\n", valExpr, k)
	}
	...
	for _, k := range eventKeys {
		expr := eventMap[k]
		fmt.Fprintf(&state.buildBuf, "vgn.DOMEventHandlerSpecList = append(vgn.DOMEventHandlerSpecList, sve.DOMEventHandlerSpec{\n")
...
func (p *ParserGo) visitNodeComponentElement(state *parseGoState, n *html.Node) error {
	...
	keyExpr := vgIfExpr(n)
	if keyExpr != "" {
		fmt.Fprintf(&state.buildBuf, "vgcompKey := sve.CompKey{ID:0x%X, IterKey: %s}\n", compKeyID, keyExpr)
	} else {
		fmt.Fprintf(&state.buildBuf, "vgcompKey := sve.CompKey{ID:0x%X, IterKey: vgiterkey}\n", compKeyID)
	}
