...
func GenModule(m, target string, mode int) {
	...
	if m == "AndroidExtras" {
		utils.Save(utils.GoQtPkgPath(strings.ToLower(m), "utils-androidextras_android.go"), utils.Load(filepath.Join(strings.TrimSpace(utils.GoListOptional("{{.Dir}}", "github.com/p9c/sve/pkg/qt/internal", "-find", "get files dir")), "/binding/files/utils-androidextras_android.go")))
	}
