...
func GetImports(path, target, tagsCustom string, level int, onlyDirect bool) []string {
	...
	for _, l := range libs {
		...
		go func(l string) {
			...
			if strings.Contains(l, "github.com/p9c/sve/pkg/qt") && !strings.Contains(l, "qt/internal") {
				if strings.Contains(l, "github.com/p9c/sve/pkg/qt/") {
...
func GetQtStdImports() (o []string) {
	for k := range importedStd {
		o = append(o, strings.TrimPrefix(k, "github.com/p9c/sve/pkg/qt/"))
	}
	return
}
...
func QtModVersion(path string) string {
	cmd := exec.Command("go", "list", "-m", "-mod=vendor", "-f", "{{.Version}}", "github.com/p9c/sve/pkg/qt")
	cmd.Dir = path
...
func RestartWithPinnedVersion(path string) bool {
	...
	cmd = exec.Command("go", "install", "-v", "-tags=no_env", "github.com/p9c/sve/pkg/qt/cmd/...")
	cmd.Dir = path
