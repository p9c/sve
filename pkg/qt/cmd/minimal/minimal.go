...
import (
	...
	"github.com/p9c/sve/pkg/qt/binding/converter"
	"github.com/p9c/sve/pkg/qt/binding/parser"
	"github.com/p9c/sve/pkg/qt/binding/templater"
...
func Minimal(path, target, tags string) {
	if utils.UseGOMOD(path) {
		...
		if utils.QT_DOCKER() {
			cmd := exec.Command("go", "get", "-v", "-d", "github.com/p9c/sve/pkg/qt/binding/files/docs/"+utils.QT_API(utils.QT_VERSION())+"@"+cmd.QtModVersion(filepath.Dir(utils.GOMOD(path)))) //TODO: needs to pull 5.8.0 if QT_WEBKIT
			cmd.Dir = path
