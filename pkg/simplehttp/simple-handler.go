...
type SimpleHandler struct {
	...
	ParserGoPkgOpts              *parser.ParserGoPkgOpts // if set enables running ParserGoPkg with these options (requires EnableBuildAndServe)
...
func New(dir string, dev bool) *SimpleHandler {
	...
	if dev {
		...
		ret.ParserGoPkgOpts = &parser.ParserGoPkgOpts{}
...
doBuild:
...

	{
		...
		if h.ParserGoPkgOpts != nil {
			pg := parser.NewParserGoPkg(h.Dir, h.ParserGoPkgOpts)
			err := pg.Run()
			if err != nil {
